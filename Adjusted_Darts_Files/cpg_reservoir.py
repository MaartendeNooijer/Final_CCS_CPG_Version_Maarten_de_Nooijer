...
class CPG_Reservoir(ReservoirBase):
    ...
    def discretize(self, verbose: bool = False) -> conn_mesh:
        ...
        #NEW
        # Apply fault multipliers BEFORE slicing mpfa_tran
        if self.faultfile is not None:
            print(f"Applying faults from: {self.faultfile}")
            self.apply_fault_mult(self.faultfile, cell_m, cell_p, mpfa_tran, ids)
            #self.discretizer.write_tran_cube('tran_faultmult.grdecl', 'nnc_faultmult.txt')

        # Now extract tran AFTER modifying mpfa_tran
        tran = np.fabs(mpfa_tran[::2][ids])
        tranD = np.fabs(mpfa_tranD[::2][ids])
    ...
    def apply_fault_mult(self, faultfile, cell_m, cell_p, mpfa_tran, ids): #NEW
        ...
        with open(faultfile) as f:
            for buff in f:
                ...
                # Stop reading EDITNNC block on new keyword
                if line.startswith((
                        'MULTIPLY', 'ENDFLUXNUM', 'REGIONS', 'EQLDIMS', 'BOX', 'ENDBOX', 'COPY')):
                    reading_editnnc = False

                # Detect start of EDITNNC block
                ...
                if reading_editnnc:
                    ...
                    # Make sure the line has enough entries
                    if len(strline) < 7:
                        continue
                    ...
                    if len(res) > 0:
                        ...
                        print(f"✔ Applied fault multiplier {fault_tran_mult} to connection ID {idx}")

    # def apply_fault_mult(self, faultfile, cell_m, cell_p, mpfa_tran, ids): #Original
    #     # Faults
    #
    #     keep_reading = True
    #     prev_fault_name = ''
    ...
    #             i2 = int(strline[4])
    #             j2 = int(strline[5])
    #             k2 = int(strline[3]) #was k2 = int(strline[6])
