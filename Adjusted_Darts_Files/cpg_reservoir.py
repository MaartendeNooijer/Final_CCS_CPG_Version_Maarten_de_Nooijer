...
class CPG_Reservoir(ReservoirBase):
    ...
    def set_arrays(self, arrays):
        ...
        self.satnum = arrays['SATNUM']  # NEW, satnum array, nx*ny*nz elements
        self.op_num = self.satnum #self.satnum.astype(np.int32) # NEW, not sure if this works
        # self.op_num = self.op_num.astype(np.int32)
    ...
    def discretize(self, verbose: bool = False) -> conn_mesh:
        ...
        self.global_data = {'volume': self.volume_all_cells[:self.discr_mesh.n_cells],
                            ...
                            'satnum': self.satnum, 'op_num': self.op_num}
    ...
    def discretize_cpg(self):
        ...
        # self.discretizer.set_porosity(self.discr_mesh_satnum)
        # self.mesh.satnum = darts.engines.value_vector(self.discretizer.poro)
        # self.satnum = np.array(self.discr_mesh_satnum, copy=False)

        # self.discretizer.set_porosity(self.discr_mesh.op_num) #NEW
        # self.mesh.op_num = darts.engines.value_vector(self.discretizer.poro)
        # self.op_num = np.array(self.discr_mesh_op_num, copy=False)
...
def read_arrays(gridfile: str, propfile: str):
    ...
    # ✅ Load SATNUM (Facies Classification) #NEW
    satnum_cpp = index_vector_discr()  # NEW
    arrays['SATNUM'] = np.array([])
    if arrays['SATNUM'].size == 0:
        load_single_int_keyword(satnum_cpp, propfile, 'SATNUM', -1)  # Load SATNUM as integers
        arrays['SATNUM'] = np.array(satnum_cpp, copy=False)  # NEW
    if arrays['SATNUM'].size == 0:  # NEW
        raise ValueError("SATNUM property not found in the properties file!")  # NEW
